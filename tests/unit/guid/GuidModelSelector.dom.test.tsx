...
describe('GuidModelSelector Codex display', () => {
  ...
  it('keeps model and reasoning controls in one menu without repeating reasoning on ordinary Home', async () => {
    ...
    expect(selector).toHaveTextContent('6 Astra 最高');
    ...
    expect(screen.getByTestId('opl-codex-session-menu-reasoning-choice-xhigh')).toHaveAttribute(
      'aria-checked',
      'false'
    );
    ...
    expect(setCodexModelSelection).toHaveBeenCalledWith('gpt-6-astra', 'high');
