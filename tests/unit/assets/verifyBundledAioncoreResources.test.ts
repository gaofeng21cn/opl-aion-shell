...
describe('verifyBundledAioncoreResources', () => {
  ...
  it('passes for the exact macOS arm64 layout', () => {
    ...
    expect(result.checked).toEqual(
      expect.arrayContaining([
        ...
        'bundled-aioncore/darwin-arm64/managed-resources/cli/codex/0.156.1/darwin-arm64/vendor/aarch64-apple-darwin/bin/codex',
      ])
    );
  });
  ...
  it('reports a missing direct Codex executable', () => {
    ...
    expect(result.missing).toContain(
      'bundled-aioncore/win32-x64/managed-resources/cli/codex/0.156.1/win32-x64/vendor/x86_64-pc-windows-msvc/bin/codex.exe'
    );
  });
  ...
  it('rejects extra on-disk direct CLI versions', () => {
    ...
    expect(result.invalid).toContain(
      'bundled-aioncore/win32-x64/managed-resources/manifest.json: codex CLI directory versions do not match 0.156.1'
    );
  });
