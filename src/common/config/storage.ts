...
export interface IConfigStorageRefer {
  ...
  'acp.config': {
    [backend in AcpBackend]?: {
      ...
      /** Preferred model ID for new conversations / 新会话的默认模型 */
      preferredModelId?: string;
      /** LLM prompt timeout in seconds (default: 43200) / LLM 请求超时时间（秒，默认 43200） */
      promptTimeout?: number;
    };
  };
  /** Global LLM prompt timeout in seconds (default: 43200). Per-backend promptTimeout overrides this. */
