...
This directory is the One Person Lab App `aionui` shell adapter, not the App repository root and not the upstream AionUI mainline.
...
- Start App product work, bug fixes, release work, and worktrees from `origin/main`.
...
- When syncing upstream AionUI, first create an explicit upstream-sync branch and then merge or replay the vetted delta back into the App product mainline.
...
- Start the intake from a clean App product baseline on `origin/main`, using a short-lived branch/worktree named like `codex/aionui-upstream-intake-YYYYMMDD`.
...
- After validation, absorb the vetted result back into `origin/main`, push when requested or when release work requires it, and clean up temporary worktrees/branches.
