...
**用户故事**：作为用户，我希望除了关于页面按钮外，还能通过应用菜单触发检查更新；同时希望应用启动时自动检查是否有新版本，并在不打断当前工作的情况下完成下载。
...
| 启动时自动检查 | `autoUpdater.checkForUpdates()` + `autoDownload=true` | 非阻塞，不影响启动 |

**启动时自动检查**：

1. 应用启动后延迟触发一次后台更新检查（非阻塞，不影响启动速度）
2. 启动检查前重置 `allowDowngrade = false`，防止前次预发布设置残留
3. 若有新版本可用，electron-updater 在后台下载；下载完成后通过 IPC broadcast 通知渲染进程
4. 渲染进程只在下载完成后显示轻量提示，用户可立即重启更新，也可稍后退出时自动安装
5. 若没有新版本或检查失败，无任何用户可见行为
