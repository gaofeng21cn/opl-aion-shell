...
export function useAcpConfigOptions({
  ...
  const { data, mutate, isLoading } = useSWR<AcpConfigOptionDto[] | null>(enabled ? key : null, fetchConfigOptions, {
    revalidateOnMount: false,
  });
  const configOptions = enabled ? (data ?? null) : null;
  ...
  const setConfigOption = useCallback(
    async (optionId: string, value: string) => {
      ...
      try {
        ...
        if (response.confirmation === 'command_ack') {
